...
func SuggestMeanGasTipCap(tip *big.Int, batch []*userop.UserOperationV06) *big.Int {
	sum := big.NewInt(0)
...
// the entire batch.
func SuggestMeanGasFeeCap(basefee *big.Int, tip *big.Int, batch []*userop.UserOperationV06) *big.Int {
...
// entire batch.
func SuggestMeanGasPrice(gasPrice *big.Int, batch []*userop.UserOperationV06) *big.Int {
	sum := big.NewInt(0)
	for _, op := range batch {
		sum = big.NewInt(0).Add(sum, op.MaxFeePerGas)
	}
